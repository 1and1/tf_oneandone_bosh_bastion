...
resource "null_resource" "bosh-env" {
  ...
  provisioner "remote-exec" {
    inline = [
       ...
       "bosh create-env /opt/bosh-deployment/bosh.yml --state=state.json --vars-store=creds.yml -o /opt/bosh-deployment/1and1/cpi.yml -v director_name=${var.director_name} -v internal_cidr=${var.bosh_subnet} -v internal_gw=${cidrhost(var.bosh_subnet, 1)} -v internal_ip=${cidrhost(var.bosh_subnet, 2)} -v private-network-id=${oneandone_private_network.bosh_net.id} -v private_key=/root/.ssh/id_rsa -v api_token=${var.api_token} -v ssh_key=\"$(cat /root/.ssh/id_rsa.pub)\""
     ]
  }
}
